...
CREATE OR REPLACE FUNCTION "mod_wms"."handle_archive_shipment_on_status_loaded"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
...
ALTER FUNCTION "mod_wms"."handle_archive_shipment_on_status_loaded"() OWNER TO "postgres";


COMMENT ON FUNCTION "mod_wms"."handle_archive_shipment_on_status_loaded"() IS 'On update of shipments status to loaded, sets is_archived = TRUE on the shipment itself. Includes error handling to prevent transaction rollback - shipment status update will succeed even if archiving fails.';
...
CREATE TABLE IF NOT EXISTS "mod_wms"."receipts" (
    ...
    "supplier_id" "uuid",
    CONSTRAINT "receipts_status_check" CHECK (("status" = ANY (ARRAY['pending'::"text", 'processing'::"text", 'in_transit'::"text", 'received'::"text", 'delivered'::"text", 'failed'::"text", 'completed'::"text"])))
);
...
CREATE OR REPLACE TRIGGER "trigger_archive_shipment_on_status_loaded" AFTER UPDATE OF "status" ON "mod_wms"."shipments" FOR EACH ROW WHEN ((("new"."status" = 'loaded'::"text") AND ("old"."status" IS DISTINCT FROM 'loaded'::"text"))) EXECUTE FUNCTION "mod_wms"."handle_archive_shipment_on_status_loaded"();



COMMENT ON TRIGGER "trigger_archive_shipment_on_status_loaded" ON "mod_wms"."shipments" IS 'Archives the shipment by setting is_archived = TRUE when shipment status changes to loaded. Errors are logged but do not prevent the shipment status update from completing.';
...
GRANT ALL ON FUNCTION "mod_wms"."handle_archive_shipment_on_status_loaded"() TO "anon";
GRANT ALL ON FUNCTION "mod_wms"."handle_archive_shipment_on_status_loaded"() TO "authenticated";
GRANT ALL ON FUNCTION "mod_wms"."handle_archive_shipment_on_status_loaded"() TO "service_role";
