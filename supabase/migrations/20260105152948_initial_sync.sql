...
using ((auth.uid() IS NOT NULL));
  ...
  for delete
  to public
using ((get_my_claim_text('role'::text) = 'superAdmin'::text));
  ...
  for insert
  to public
with check ((get_my_claim_text('role'::text) = 'superAdmin'::text));
...
using ((get_my_claim_text('role'::text) = 'superAdmin'::text));
  ...
  for delete
  to public
using ((get_my_claim_text('role'::text) = ANY (ARRAY['admin'::text, 'superAdmin'::text])));
  ...
  for insert
  to public
with check ((get_my_claim_text('role'::text) = ANY (ARRAY['admin'::text, 'superAdmin'::text])));
...
using (true);
  ...
  for all
  to public
using (((get_my_claim_text('role'::text) = 'admin'::text) AND (((get_my_claim_text('domain_id'::text))::uuid = domain_id) OR mod_admin.is_subdomain(domain_id, (get_my_claim_text('domain_id'::text))::uuid))));
  ...
  for all
  to public
using (((get_my_claim_text('domain_id'::text) = ANY (shared_with)) OR (('*'::text = ANY (shared_with)) AND mod_admin.is_subdomain(domain_id, (get_my_claim_text('domain_id'::text))::uuid))));
...
using ((( SELECT auth.uid() AS uid) IS NOT NULL));
  ...
  for all
  to public
using ((COALESCE(get_my_claim_text('role'::text), ''::text) = 'superAdmin'::text));
  ...
  for all
  to public
using (((get_my_claim_text('domain_id'::text))::uuid = domain_id));
...
with check (((( SELECT auth.uid() AS uid) IS NOT NULL) AND ((domain_id IN ( SELECT domain_users.domain_id
  ...
  WHERE (domain_users.user_id = auth.uid()))) OR (get_my_claim_text('role'::text) = ANY (ARRAY['admin'::text, 'superAdmin'::text])) OR (get_my_claim_text('domain_id'::text) = ANY (shared_with)) OR ('*'::text = ANY (shared_with)))));
...
with check (((( SELECT auth.uid() AS uid) IS NOT NULL) AND ((domain_id IN ( SELECT domain_users.domain_id
   FROM mod_admin.domain_users
  WHERE (domain_users.user_id = auth.uid()))) OR (get_my_claim_text('domain_id'::text) = ANY (shared_with)) OR ('*'::text = ANY (shared_with)))));
...
using (((( SELECT auth.uid() AS uid) IS NOT NULL) AND ((domain_id IN ( SELECT domain_users.domain_id
   FROM mod_admin.domain_users
  WHERE (domain_users.user_id = auth.uid()))) OR (get_my_claim_text('domain_id'::text) = ANY (shared_with)) OR ('*'::text = ANY (shared_with)) OR mod_admin.is_subdomain(domain_id, (get_my_claim_text('domain_id'::text))::uuid))));
...
using (((is_deleted = false) AND (EXISTS ( SELECT 1
   FROM mod_base.departments d
  WHERE ((d.id = pages_departments.department_id) AND ((get_my_claim_text('domain_id'::text))::uuid = d.domain_id))))));
...
using (((get_my_claim_text('domain_id'::text))::uuid = domain_id));
  ...
  for select
  to public
using (((get_my_claim_text('role'::text) = 'administrator'::text) AND ((get_my_claim_text('domain_id'::text))::uuid = domain_id)));
  ...
  for select
  to public
using (((get_my_claim_text('role'::text) = 'superadmin'::text) OR (domain_id = ANY ((get_my_claim_text('shared_domains'::text))::uuid[]))));
  ...
  for insert
  to public
with check (((auth.role() = 'authenticated'::text) AND ((get_my_claim_text('domain_id'::text))::uuid = domain_id)));
  ...
  for update
  to public
using (((auth.role() = 'authenticated'::text) AND ((get_my_claim_text('domain_id'::text))::uuid = domain_id)));
...
using ((( SELECT auth.uid() AS uid) IS NOT NULL));
  ...
  for select
  to public
using (((get_my_claim_text('role'::text) = 'admin'::text) AND (((get_my_claim_text('domain_id'::text))::uuid = domain_id) OR mod_admin.is_subdomain(domain_id, (get_my_claim_text('domain_id'::text))::uuid)) AND (is_deleted = false)));
...
using ((( SELECT auth.uid() AS uid) IS NOT NULL));
  ...
  for select
  to public
using (((get_my_claim_text('role'::text) = 'superAdmin'::text) AND (is_deleted = false)));
...
using ((is_deleted = false));
  ...
  for select
  to public
using (((get_my_claim_text('domain_id'::text) = ANY (shared_with)) OR (('*'::text = ANY (shared_with)) AND (is_deleted = false))));
...
using (
CASE
    WHEN (get_my_claim_text('user_role'::text) = 'SuperAdmin'::text) THEN true
    WHEN (get_my_claim_text('user_role'::text) = 'Admin'::text) THEN ((domain_id = (get_my_claim_text('domain_id'::text))::uuid) OR (domain_id IN ( SELECT domains.id
       FROM mod_admin.domains
      WHERE (domains.parent_domain_id = (get_my_claim_text('domain_id'::text))::uuid))))
    ELSE (domain_id = (get_my_claim_text('domain_id'::text))::uuid)
END);
  ...
  for select
  to public
using (((get_my_claim_text('domain_id'::text))::uuid = ANY ((shared_with)::uuid[])));
...
with check ((is_deleted = true));
  ...
  for insert
  to authenticated
with check (((get_my_claim_text('domain_id'::text))::uuid = domain_id));
...
with check (((get_my_claim_text('domain_id'::text))::uuid = domain_id));
  ...
  for select
  to public
using ((get_my_claim_text('user_role'::text) = 'SuperAdmin'::text));
...
with check ((( SELECT auth.uid() AS uid) IS NOT NULL));
  ...
  for select
  to public
using (((auth.uid() IS NOT NULL) AND ((get_my_claim_text('domain_id'::text))::uuid = domain_id) AND (is_deleted = false)));
